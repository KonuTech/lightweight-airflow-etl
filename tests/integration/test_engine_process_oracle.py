...
def test_process_reprocess_returns_identical_result(
    ...
    # D-01: the second call returns the ORIGINAL recorded outcome -- same
    # status/counts/checksum -- but never the identical wall-clock
    # duration_seconds (each call measures its own real elapsed time), so
    # every field except duration_seconds is compared explicitly rather
    # than a blanket dataclass/model equality check.
    assert second_result.status == first_result.status
    assert second_result.dataset == first_result.dataset
    assert second_result.file_name == first_result.file_name
    assert second_result.total_rows == first_result.total_rows
    assert second_result.valid_rows == first_result.valid_rows
    assert second_result.invalid_rows == first_result.invalid_rows
